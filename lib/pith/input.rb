  ...
  class Input
    ...
    def evaluate_as_tilt_template
      if path.to_s =~ /^(.*)\.(.*)$/ && RenderContext.can_render?($2)
        ...
        full_output_path = project.output_dir + output_path
        unless uptodate?(full_output_path)
    ...
    def copy_verbatim
      trace("copy", path)
      full_output_path = project.output_dir + path
      unless uptodate?(full_output_path)
        full_output_path.parent.mkpath
    ...
    def uptodate?(output_path)
      return false unless @dependencies
      input_paths = @dependencies.map { |input| input.full_path }
      FileUtils.uptodate?(output_path, input_paths)
    end
